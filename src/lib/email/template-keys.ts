import { BATCH7_EMAIL_TEMPLATES } from "@/lib/finance/email-events";
import { APPROVED_TEMPLATE_KEYS, type TemplateKey } from "./types";

/** Existing outbox keys that are not catalog UPPER_SNAKE templates. */
export const OPERATIONAL_OUTBOX_KEYS = [
  "newsletter_campaign",
  ...BATCH7_EMAIL_TEMPLATES,
] as const;
...
export function isOperationalOutboxKey(key: string): boolean {
  return (OPERATIONAL_OUTBOX_KEYS as readonly string[]).includes(key);
}

/** Retry/process existing outbox rows (catalog keys + newsletter_campaign / batch-7). */
export function assertOutboundTemplateKey(key: string): string {
  const trimmed = key.trim();
  if (isOperationalOutboxKey(trimmed)) return trimmed;
  return assertEnqueueableTemplateKey(trimmed);
}
