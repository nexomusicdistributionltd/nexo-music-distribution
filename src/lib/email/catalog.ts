...
export const EMAIL_CATALOG: EmailCatalogEntry[] = [
  ...
  { eventType: "release.qc", templateKey: "RELEASE_CHANGES_REQUIRED", filePath: "emails/templates/RELEASE_CHANGES_REQUIRED.html", subject: "Changes required — {{RELEASE_TITLE}}" },
  { eventType: "release.qc", templateKey: "RELEASE_REJECTED", filePath: "emails/templates/RELEASE_REJECTED.html", subject: "Release rejected — {{RELEASE_TITLE}}" },
  { eventType: "release.qc", templateKey: "RELEASE_APPROVED", filePath: "emails/templates/RELEASE_APPROVED.html", subject: "Release approved — {{RELEASE_TITLE}}" },
  { eventType: "release.status", templateKey: "RELEASE_QUEUED", filePath: "emails/templates/RELEASE_QUEUED.html", subject: "Queued for distribution — {{RELEASE_TITLE}}" },
  { eventType: "release.status", templateKey: "RELEASE_DISTRIBUTING", filePath: "emails/templates/RELEASE_DISTRIBUTING.html", subject: "Distribution started — {{RELEASE_TITLE}}" },
  {
  ...
  { eventType: "release.status", templateKey: "RELEASE_FAILED", filePath: "emails/templates/RELEASE_FAILED.html", subject: "Distribution failed — {{RELEASE_TITLE}}" },
...
/** Prefer UPDATE_REQUIRED when changes are requested after approval. */
export function templateKeyForReleaseTransition(
  from: string,
  to: string
): TemplateKey | null {
  if (to === "changes_requested" && POST_APPROVAL.has(from)) {
    return "RELEASE_UPDATE_REQUIRED";
  }
  return templateKeyForReleaseStatus(to);
}

export function eventTypeForCatalogKey(key: TemplateKey): EmailCatalogEntry["eventType"] {
  return getCatalogEntry(key)?.eventType ?? "release.status";
}
