 ...
 * Optional blocks {{#VAR}}...{{/VAR}} are omitted when the value is empty.
 * Unknown placeholders are stripped — never send raw {{PLACEHOLDERS}}.
 */
export function substitutePlaceholders(
  ...
  out = out.replace(/\{\{\s*([A-Z0-9_]+)\s*\}\}/g, (_m, name: string) => {
    return safe[name] ?? "";
  });
  // Never leave unresolved tokens in outbound HTML.
