...
export async function listPublicArtists(limit = 48): Promise<PublicArtistCard[]> {
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id, artist_name, stage_name, public_slug, public_tagline, public_bio_html, avatar_url, cover_url, website_featured, website_sort_order, social_links, genres, country, entzopedia_url"
    )
    .eq("website_published", true)
    .order("website_featured", { ascending: false })
    .order("website_sort_order", { ascending: true })
...
export async function getPublicArtistBySlug(slug: string) {
  ...
  const { data, error } = await supabase
    .from("artist_profiles")
    .select(
      "id, artist_name, stage_name, public_slug, public_tagline, public_bio_html, avatar_url, cover_url, social_links, website_featured, genres, country, entzopedia_url"
    )
    .eq("public_slug", slug)
    .eq("website_published", true)
    .maybeSingle();
