...
export function IdentityVerificationFlow({userId,initial}:{userId:string;initial?:{id:string;status:string;country_code:string;legal_full_name:string;date_of_birth:string;document_type:DocumentType}|null}){
 ...
 async function chooseDocument(next:DocumentType){setBusy(true);setMessage("");const r=await saveVerificationIdentityAction({countryCode:country,legalFullName:name,dateOfBirth:dob,documentType:next});setBusy(false);if(!r.ok||!r.id){setMessage(r.error??"Could not save document type.");return;}setDoc(next);setVerificationId(r.id);setStep(3);}
 ...
 return <div className="mx-auto max-w-2xl space-y-6">
   ...
   {step===2?<section className="space-y-4 rounded-xl border border-[var(--nexo-border)] bg-[var(--nexo-surface)] p-5"><h2 className="text-h4">Select your document</h2>
     {([...(country==="NG"?([["nin","NIN"]] as const):[]),["national_id","ID Card"],["drivers_license","Driver’s License"],["passport","International Passport"]] as const).map(([v,l])=><button key={v} disabled={busy} onClick={()=>void chooseDocument(v)} className="flex w-full items-center justify-between rounded-lg border border-[var(--nexo-border)] p-4 text-left hover:bg-[var(--nexo-ghost-hover)]"><span>{l}</span><span>Continue →</span></button>)}
   </section>:null}
