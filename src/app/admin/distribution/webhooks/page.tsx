...
export default async function WebhooksPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Webhooks / Events"
        description="Idempotent provider webhook log. Rejected when signature secret missing."
      />
      ...
      <p className="mt-2 text-small text-[var(--nexo-text-muted)]">
        Endpoint: POST /api/webhooks/provider — webhook secret{" "}
        {provider.webhookConfigured ? "configured" : "missing (fail closed)"}
      </p>
      {events.length === 0 ? (
        <div className="mt-4">
