-- Canonical outbox helpers on public.email_outbound_events only.
-- Does not create email_events. Rejects untrusted providers including resend/null/fake.
...
comment on table public.email_outbound_events is
  'Canonical outbound email outbox. status=sent only after zoho-smtp (or other real provider) accept + provider_message_id.';
...
begin
  if new.status = 'sent' then
    ...
    if lower(trim(new.provider)) in ('none', 'not_connected', 'fake', 'test', 'null', 'resend', 'sendgrid', 'mailgun', 'postmark') then
...
begin
  ...
  if canonical = 'sent' then
    ...
    if p_provider is null or length(trim(p_provider)) = 0
       or lower(trim(p_provider)) in ('none', 'not_connected', 'fake', 'test', 'null', 'resend', 'sendgrid', 'mailgun', 'postmark') then
